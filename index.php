...
if (($mysqli instanceof mysqli) && !$mysqli->connect_errno) {
	...
	$featured_case_id = (is_array($weekly_featured_case) && isset($weekly_featured_case['id'])) ? intval($weekly_featured_case['id']) : 0;
	...
	$seen_sql = "SELECT id,title,thumbnail,source_id,category_id,datetime,hits FROM news WHERE published='1' AND thumbnail<>''";
	if ($featured_case_id > 0) {
		$seen_sql .= " AND id<>" . $featured_case_id;
	}
	$seen_sql .= " ORDER BY RAND() LIMIT 1";
	...
	} elseif ($featured_case_id > 0) {
		$fallback_seen_sql = "SELECT id,title,thumbnail,source_id,category_id,datetime,hits FROM news WHERE published='1' AND thumbnail<>'' AND id=" . $featured_case_id . " LIMIT 1";
		$fallback_seen_query = $mysqli->query($fallback_seen_sql);
		if ($fallback_seen_query && $fallback_seen_query->num_rows > 0) {
			$fallback_row = $fallback_seen_query->fetch_assoc();
			$fallback_row['thumb_url'] = rss_article_thumb_url(isset($fallback_row['thumbnail']) ? $fallback_row['thumbnail'] : '', $siteurl);
			if ($fallback_row['thumb_url'] !== '') {
				$seen_people_cases[] = $fallback_row;
			}
		}
	}
}

    ...
    public function getAttributes($compiler, $attributes)
    {
        ...
        foreach ($attributes as $key => $mixed) {
            ...
            if (!is_array($mixed)) {
                ...
                $kv_key = key($mixed);
                $kv_value = $kv_key !== null ? $mixed[$kv_key] : null;
                // option flag?
                if (in_array($kv_key, $this->option_flags)) {
                    if (is_bool($kv_value)) {
                        $_indexed_attr[$kv_key] = $kv_value;
                    } elseif (is_string($kv_value) && in_array(trim($kv_value, '\'"'), array('true', 'false'))) {
                        if (trim($kv_value) == 'true') {
                            $_indexed_attr[$kv_key] = true;
                        } else {
                            $_indexed_attr[$kv_key] = false;
                        }
                    } elseif (is_numeric($kv_value) && in_array($kv_value, array(0, 1))) {
                        if ($kv_value == 1) {
                            $_indexed_attr[$kv_key] = true;
                        } else {
                            $_indexed_attr[$kv_key] = false;
                        ...
                        $compiler->trigger_template_error("illegal value of option flag \"{$kv_key}\"", $compiler->lex->taglineno);
                    }
